...
async function init(){
	...
	let currenUserScore = await scoreActions.getScore(location.hostname,WA.player.id).then(data => data.json());
	...
	var player = {
		...
		applyAmountUpdate() {
			...
			scoreActions.addScore(location.hostname,this.id, diff).then(() => {
